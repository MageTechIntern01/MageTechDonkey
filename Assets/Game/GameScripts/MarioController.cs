...
public class MarioController : MonoBehaviour {
	...
	void jump (){
		if (grounded && CrossPlatformInputManager.GetButtonDown("Jump") && canJump){
			jumpClip.Play();
	...
	void LadderClimb() {
		...
		if (ladder && !hammer){
			...
			float climbVel = 1.5f * CrossPlatformInputManager.GetAxisRaw("Vertical");
	...
	public void SetStats(Transform Position)
    {
		transform.position = new Vector2(Position.position.x + 0.8f,Position.position.y);
		endGame = true;
		myRigidbody2D.velocity = Vector2.zero;
		anim.SetFloat("Speed", 0);
	}
